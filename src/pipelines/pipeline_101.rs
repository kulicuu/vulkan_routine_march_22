    ...
    &'a str>
{
    ...
    let depth_image = device.lock().unwrap().create_image(&depth_image_info, None)
            .expect("Failed to create depth (texture) Image.");   

    let dpth_img_mem_reqs = device.lock().unwrap().get_image_memory_requirements(depth_image);
    let dpth_img_mem_info = vk::MemoryAllocateInfoBuilder::new()
        .memory_type_index(1)
        .allocation_size(dpth_img_mem_reqs.size);
    let depth_image_memory = device.lock().unwrap().allocate_memory(&dpth_img_mem_info, None)
        .expect("Failed to alloc mem for depth image.");

    device.lock().unwrap().bind_image_memory(depth_image, depth_image_memory, 0)
    ...
    let depth_image_view = device.lock().unwrap().create_image_view(&depth_image_view_info, None)
    ...
    let shader_vert = device.lock().unwrap().create_shader_module(&module_info, None).unwrap();
    ...
    let shader_frag = device.lock().unwrap().create_shader_module(&module_info, None).unwrap();
